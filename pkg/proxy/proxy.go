...
func (p *Proxy) forwardRequest(acct *account.Account, w http.ResponseWriter, req *http.Request) {
	...
	for {
		...
		body := make([]byte, MaxResponseLength+1)
		body, err = inet.ReadWithContext(ctx, result.Body, body)
		result.Body.Close()
		...
		if result.StatusCode == http.StatusOK {
			...
			w.Write(body)
			return
		} else if result.StatusCode == http.StatusMisdirectedRequest && result.Header.Get("Alt-Svc") != "" {
		...
		log.Debug("Retrying transmission after error: %s", body)
		select {
		case <-ctx.Done():
...
func (p *Proxy) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	if p.fetchDomainForSubject(acct.Subject) != "" {
		acct.Host = p.fetchDomainForSubject(acct.Subject)
	}
